          ...
          {tutors.map((t, i) => (
            ...
            >
              ...
              <div className="flex justify-between items-center mb-4">
                <span className="text-sm text-green-600 font-semibold">★ {t.rating}</span>
                <span className="font-bold text-indigo-500">€{t.price}/hr</span>
              </div>
      ...
      <motion.section variants={fadeUp} initial="hidden" animate="visible" custom={8} className="mb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} className="bg-background border border-border rounded-2xl p-5 hover:border-indigo-300 transition-all">
